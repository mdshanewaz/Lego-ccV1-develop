...
from resumeweb.send_email_v2 import send_email
...
from resumeweb.product_selection import (
...
from cloudops.fetch_data import fetch_data_ta


PATH_ROOT               = os.path.join(settings.BASE_DIR)
PATH_PRODUCT_LAYOUT     = 'adhoc/layout/'
...
class AdhocHome(ListView):
    ...
    queryset      = mprod_exp180.objects.all() #.filter(adhoc_get_started='yes')
...
class AdhocRequestByGuest(CreateView):
    ...
    def form_valid(self, form, **kwargs):
        ...
        coupon_code_dict = generate_coupon(product_liked="mprod_adhoc")
    ...
    #         'proglang_list': mprod_proglang.objects.distinct('langname'),
...
class AdhocUseCases(ListView):
    ...
    def get_queryset(self):
        return mprod_exp180.objects.all().filter(level='adhoc_get_started')
