...
PATH_PRODUCT_LAYOUT     = 'resumeweb/layout/products/5-staircase/'
PATH_ROOT               = os.path.join(settings.BASE_DIR)
PATH_PRODUCT_COMPONENT  = 'resumeweb/templates/resumeweb/components/products/staircase'

PRODUCT_LINE        = 'staircase'
PRODUCT_SLOGAN      = 'add slogan herestaircase'
MODEL_PRODUCT       = mprod_proflevel
MODEL_PRODUCT_CAT   = mprod_proflevel_list


# ******************************************************************************
class ProfLevelHome(ListView):
...
# ******************************************************************************
class ProfLevelServiceAll(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        ## special items
        # cat list section
        context['cat_list'] = MODEL_PRODUCT_CAT.objects.values_list('levelname', flat=True).distinct()
...
# ******************************************************************************
class ProfLevelServiceByCat(ListView):
    template_name = PATH_PRODUCT_LAYOUT + "all_serv.html"
    model = MODEL_PRODUCT

    def get_queryset(self):
        qs1 = super().get_queryset()
        category_selected = get_object_or_404(MODEL_PRODUCT_CAT, levelname=self.kwargs.get('levelname'))
    ...
    def get_context_data(self, **kwargs):
        ...
        category_selected = self.kwargs['levelname']
...
def populate_cardDetailViewClass():
    product_model_name              = "mprod_proflevel"
    product_detail_template_name    = "resumeweb/pg-contents/product-details-v2/prod-details.html"
    breadcrumb_url_name             = "proflevel_all_services"     # Name of django view that is used for breadcrumb url for this product
...
# ******************************************************************************
def ProfLevelServiceDetails_ajax(request, id):
    zzz_print("    %-28s: %s" % ("ProfLevelServiceDetails_ajax (id)", id))
...
# ******************************************************************************
def ProfLevelServiceDetails(request, id):
    zzz_print("    %-28s: %s" % ("ProfLevelServiceDetails (id)", id))
...
# how it works
# ******************************************************************************
class ProfLevelHowItWorks(TemplateView):
...
# use cases
# ******************************************************************************
class ProfLevelUseCases(TemplateView):
    template_name = PATH_PRODUCT_LAYOUT + "prod_ins_pg.html"
...
# Product Comparison
# ******************************************************************************
class ProfLevelCompareThis(TemplateView):
    template_name = "resumeweb/layout/base_prod_comparison_v2.html"

    def get_context_data(self, *args, **kwargs):
        context = super().get_context_data(**kwargs)
        
        prod_comp_data = [i[1] for i in prod_comp_data_staircase]
...
# Delivery method
# ******************************************************************************
class ProfLevelProductDeliveryMethods(TemplateView):
    template_name = PATH_PRODUCT_LAYOUT + "prod_ins_pg.html"
...
# Product faq
# ******************************************************************************
class ProfLevelFaq(TemplateView):
    template_name = PATH_PRODUCT_LAYOUT + "prod_ins_pg.html"
