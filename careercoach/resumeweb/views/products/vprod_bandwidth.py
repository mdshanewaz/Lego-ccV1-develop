...
PATH_PRODUCT_LAYOUT     = "resumeweb/layout/products/8-bandwidth/"
PATH_PRODUCT_COMPONENT  = 'resumeweb/templates/resumeweb/components/products/bandwidth'
PRODUCT_LINE            = 'bandwidth'
PRODUCT_SLOGAN          = 'add slogan hereidentity'
MODEL_PRODUCT           = mprod_visabased
MODEL_PRODUCT_CAT       = mprod_visabased_list


# ******************************************************************************
class USvisaHome(ListView):
...
# ******************************************************************************
class USvisaServiceAll(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        ## special items
        # cat list section
        cat_list = MODEL_PRODUCT_CAT.objects.values_list('visaname', flat=True).distinct()
...
# ******************************************************************************
class USvisaServiceByCat(ListView):
    template_name = PATH_PRODUCT_LAYOUT + "all_serv.html"
    model = MODEL_PRODUCT

    def get_queryset(self):
        qs1 = super().get_queryset()
        category_selected = get_object_or_404(MODEL_PRODUCT_CAT, visaname=self.kwargs.get('visaname'))
    ...
    def get_context_data(self, **kwargs):
        ...
        category_selected = self.kwargs['visaname']
...
def populate_cardDetailViewClass():
    product_model_name              = "mprod_visabased"
    product_detail_template_name    = "resumeweb/pg-contents/product-details-v2/prod-details.html"
    breadcrumb_url_name             = "usvisa_all_services"     # Name of django view that is used for breadcrumb url for this product
    breadcrumb_url_displaytext      = "Bandwidth All Services"     # Text to display in breadcrumb url
...
# ******************************************************************************
def USVisaProdDetails_ajax(request, id):
    zzz_print("    %-28s: %s" % ("USVisaProdDetails_ajax (id)", id))
...
# ******************************************************************************
def USVisaProdDetails(request, id):
    zzz_print("    %-28s: %s" % ("USVisaProdDetails (id)", id))
...
# ******************************************************************************
class USvisaOurspeciality(TemplateView):
...
# ******************************************************************************
# job lost support
class JobLostSupport(TemplateView):
    template_name = PATH_PRODUCT_LAYOUT + "job-lost-support.html"



# ******************************************************************************
class USvisaTypeDetails(TemplateView):
    pass 


# how it works
# ******************************************************************************
class USvisaHowItWorks(TemplateView):
    template_name = PATH_PRODUCT_LAYOUT + "prod_ins_pg.html"
...
# product Comparison
# ******************************************************************************
class USvisaCompareThis(TemplateView):
    template_name = "resumeweb/layout/base_prod_comparison_v2.html"

    def get_context_data(self, *args, **kwargs):
        context = super().get_context_data(**kwargs)
        
        prod_comp_data = [i[1] for i in prod_comp_data_bandwidth]
...
# use cases
# ******************************************************************************
class USvisaUseCases(TemplateView):
    template_name = PATH_PRODUCT_LAYOUT + "prod_ins_pg.html"
...
# ******************************************************************************
# delivery method
class USvisaProductDeliveryMethods(TemplateView):
    template_name = PATH_PRODUCT_LAYOUT + "prod_ins_pg.html"
