...
from ..models import mcoupon_given
from ..models import (mcart_coupons, BuyerListGuest)
from ..forms import (
    mmhCouponForm, 
    mmhFileUploadForm, 
    mmhEmailForm,
...
from ..send_email_v3 import send_email

TEMP_DIR                 = 'resumeweb/layout/'
TEMP_DIR_SHOPCART        = 'resumeweb/layout/shoppingcart_v2/'
TEMP_DIR_SHOPCART_EMAIL  = 'resumeweb/email_templates/shopping_cart/'
...
def mmh_CartHome_contents(request, mode):
            ...
            if not imcoupon_given:
                ...
                if imcoupon_given.is_coupon_expired():
                    ...
                    if imcoupon_given.usedinpurchase:
                        ...
                        if imcoupon_given.mcoupon.product_liked not in ['mprod_xyz', 'mprod_adhoc', 'mprod_exp180', 'mprod_intprep', 'mprod_proflevel', 'mprod_proglang', 'mprod_rolebased', 'mprod_strategy', 'mprod_visabased']:
    ...
    # from resumeweb.views.cart_context import cart_context_loggit
    ...
    context['coupon_form'] = render_to_string(
        TEMP_DIR_SHOPCART+ 'components200/' + 'coupon/' +'form.html',
        {'form':form, 'form_errors':form_errors},
        request=request
    )

    html = render_to_string(TEMP_DIR_SHOPCART+'components200/'+'cart_contents_all.html', context)
...
def mmh_CartHome(request):
    ...
    template = loader.get_template(TEMP_DIR+'base_shopcart.html')
    context = {
...
def mmh_CartFileUpload(request):
    ...
    if 'mmh_failed_validation' in request.session:
        file_upload_failure_modal = render_to_string(TEMP_DIR_SHOPCART+'components200/file_upload/'+'file_uploadfailure_modal_trigger.html', {}, request=request)
    ...
    context             = cart_context_forUser(request)
    cart_summary_html   = render_to_string(TEMP_DIR_SHOPCART+'components200/summary/sum101.html', context)
    ...
    if context['products_list']:
        file_upload_form_html = render_to_string(TEMP_DIR_SHOPCART+'components200/file_upload/'+'form610.html', {'form':form}, request=request)

    template = loader.get_template(TEMP_DIR_SHOPCART+'file_upload.html')
    context = {
...
def mmh_CartUserLogin(request):
    ...
    if request.method == 'GET':
        ...
        form = mmhSignInForm()
        return {'html': render_to_string(TEMP_DIR_SHOPCART+ 'components200/user_login/' +'form_reguser.html', {'form': form}, request)}
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                zzz_print("    %-28s: %s" % ("email", email))
                zzz_print("    %-28s: %s" % ("password", password))
                return {'html': render_to_string(TEMP_DIR_SHOPCART+'components200/user_login/' +'form_reguser.html', {'form': form, "form_errors": form_errors}, request)}
        else:
            zzz_print("    %-28s: %s" % ("", "form.is NOT valid()"))
            return {'html': render_to_string(TEMP_DIR_SHOPCART+'components200/user_login/' +'form_reguser.html', {'form': form}, request)}
    ...
    if request.method == 'GET':
        ...
        context = {
            'html': render_to_string(
                    TEMP_DIR_SHOPCART+ 'components200/' + 'user_login/' +'form_guest.html', 
                    {'form': form}, 
                    request = request
                )
        }
...
def mmh_CartLoginPage(request):
    ...
    template = loader.get_template(TEMP_DIR_SHOPCART+'user_login.html')
...
def mmh_CartLoginOrGuestSuccess(request):
    ...
    template = loader.get_template(TEMP_DIR_SHOPCART+'paypal_checkout.html')

    context                         = cart_context_forUser(request)
    cart_summary_html               = render_to_string(TEMP_DIR_SHOPCART+'components200/summary/' +'sum101.html', context)
    paypal_buttons_container_html   = render_to_string(TEMP_DIR_SHOPCART+'components200/checkout/'+'paypal_buttons_container.html', {})
...
def mmh_CartPurchaseSuccess(request, paypaltransaction_id):
    ...
    template = loader.get_template(TEMP_DIR_SHOPCART+'purchase_conf.html')
    ...
    html_message_text = render_to_string(
        template_name=TEMP_DIR_SHOPCART_EMAIL + 'test.html',
        context=cart_context,
        using=None,
        request=None
    )
    plain_message_text = strip_tags(html_message_text)
    send_email(subject, plain_message_text, html_message_text, to_emails_list, appname)
    # email function ends
    ...
    context = {
        ...
        'shopping_cart_contents'    : render_to_string(TEMP_DIR_SHOPCART+'components200/'+'cart_contents_all.html', cart_context),
        'templatename'              : template.origin.__str__().rsplit('/', 1)[1],
    }
    return HttpResponse(template.render(context, request))
...
def rwCartAjaxView_carticon(request):
    ...
    html = render_to_string('resumeweb/components/menus/header/cart_button_02072022.html', context)
    # zzz_print("    %-28s: %s" % ("html", html))
...
def does_product_model_name_require_resume_file_upload(model_name):
    ...
    if model_name in const_resumeweb.MODELNAME_BOOLEAN_DICT_PRODUCTS_THAT_WORK_IN_CART_SYSTEM_RESUME_REQUIRED:
        zzz_print("    %-28s: %s" % ("model_name FOUND in dict", model_name))
        return const_resumeweb.MODELNAME_BOOLEAN_DICT_PRODUCTS_THAT_WORK_IN_CART_SYSTEM_RESUME_REQUIRED[model_name]
    else:
