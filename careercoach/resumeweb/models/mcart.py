...
from .muniqueid import muniqueid_get_users_uniquid
...
class mcart_instancemanager(models.Manager):
    ...
    def mcart_add_or_update_ver2(self, request, item_id, model_name, quantity, service_option_id_list, delivery_option_id_list, resume_required):
        ...
        product_dynamic_django_model        = apps.get_model("resumeweb", product_model_name)
...
class mcart(models.Model):
    ...
    processing_status       = models.CharField      (max_length=20, choices=const_resumeweb.PURCHASE_STATUS, default='pending')
    mcompleted_purchase     = models.ForeignKey     ('resumeweb.mcompleted_purchase', blank=True, null=True, on_delete=models.CASCADE)
    mcompleted_refund       = models.ForeignKey     ('resumeweb.mcompleted_refund', blank=True, null=True, on_delete=models.CASCADE)
    ...
    def get_user_visible_product_line(self):
        if self.model_name in const_resumeweb.MODELNAME_DISPLAYNAME_DICT_PRODUCTS_THAT_WORK_IN_CART_SYSTEM:
            return const_resumeweb.MODELNAME_DISPLAYNAME_DICT_PRODUCTS_THAT_WORK_IN_CART_SYSTEM[self.model_name]
        else:
