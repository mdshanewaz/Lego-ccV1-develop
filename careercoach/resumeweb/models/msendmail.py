...
class msendmail_instancemanager(models.Manager):
    # --------------------------------------------------------------------------
    def add(self, subject, plain_message, html_message, from_email, to_emails_list, appname):
        # zzz_print("    %-28s: %s" % ("to_emails_list orig", to_emails_list))
        # Remove duplicates from to_emails_list
        to_emails_list = list(dict.fromkeys(to_emails_list))
        # zzz_print("    %-28s: %s" % ("to_emails_list after", to_emails_list))

        # convert list of emails to string
        to_emails = ','.join(to_emails_list)
...
class msendmail(models.Model):
    ...
    def _send(self, recipient_string):
        ...
        try:
            ...
            elif self.appname == 'PRODSERV':
                auth_user           = settings.EMAIL_CONFIG["PRODSERV"]["EMAIL_HOST_USER"]
                auth_password       = settings.EMAIL_CONFIG["PRODSERV"]["EMAIL_HOST_PASSWORD"]
            ...
            if return_value == 0:
                ...
                fail_title              = "Django send_mail failed."
